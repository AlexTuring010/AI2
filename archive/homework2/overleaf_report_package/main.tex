...
Το καλύτερο public Kaggle αποτέλεσμα ήταν 0.70 macro-F1 στο public leaderboard με username \textbf{AlexTuring - sdi2200284}. Στο τοπικό μου validation set, το καλύτερο score έφτασε 0.7136 macro-F1, με ensemble τριών διαφορετικών εκδοχών του ίδιου DeBERTa μοντέλου. Το ότι το Kaggle πέφτει λίγο σε σχέση με το local val είναι αναμενόμενο -- δεν είναι ακριβώς ίδιο data distribution, οπότε αυτό μου έδωσε και μια ρεαλιστική ιδέα του πόσο γενικεύει το μοντέλο μου.
...
\subsection{Kaggle notebooks και leaderboard}

Τα τρία τελικά Kaggle notebooks που αντιστοιχούν στα απαιτούμενα μοντέλα είναι:

\begin{itemize}
    \item BERT: \href{https://www.kaggle.com/code/sdi2200284/final-bert}{\texttt{final-bert}}
    \item DeBERTa: \href{https://www.kaggle.com/code/sdi2200284/final-deberta}{\texttt{final-deberta}}
    \item DistilBERT: \href{https://www.kaggle.com/code/sdi2200284/final-distrilbert/edit}{\texttt{final-distrilbert}}
\end{itemize}

Στο public leaderboard εμφανίζομαι ως \textbf{AlexTuring - sdi2200284}. Το public αποτέλεσμα που κρατάω ως τελικό για την αναφορά είναι \textbf{0.70 macro-F1}.

Για να μη μπερδευτεί αυτό με τα πολλά DeBERTa πειράματα: το τελικό Kaggle submission ήταν το \textbf{Phase 12 DeBERTa + numeric side branch alpha ensemble}. Δηλαδή raw full question--answer input, τα 19 numeric features στο side branch, seeds 42/0/1 και fixed weights [0.9, 0.0, 0.1]. Το Phase 18 είχε καλύτερο local validation score, αλλά \textbf{δεν} ήταν αυτό που επέλεξα για το final Kaggle submission.
...
\textbf{DeBERTa + numeric side branch} & DeBERTa pooled vec + handcrafted feats via MLP & \textbf{0.7007} & \textbf{0.70} \\
...
\bottomrule
\end{tabularx}
\caption{Συνολικός πίνακας πειραμάτων. Το final submitted Kaggle score είναι 0.70.}
\label{tab:trials}
\end{table}
...
Αποτέλεσμα: \textbf{0.7007} (seed=42, single model). +0.005 πάνω από το vanilla DeBERTa, και κρίσιμα η Clear Reply F1 ανέβηκε αρκετά (από 0.611 σε 0.660). Με α-weighted 3-seed ensemble: 0.7057. Στο Kaggle αυτό ήταν το final submission: \textbf{0.70}.
...
Το καλύτερο μου submitted Kaggle αποτέλεσμα (0.70 public macro-F1) προέρχεται από αυτή την αρχιτεκτονική:
...
Local val macro-F1 αυτής της εκδοχής: 0.7057. Public Kaggle: \textbf{0.70}.

Η ακόμα καλύτερη τοπική εκδοχή (Phase 18 alpha ensemble) έφτασε 0.7136 val, συνδυάζοντας raw + word-normalized + focused views, αλλά δεν την υπέβαλα ως τελικό γιατί ήθελα να κρατήσω το αξιόπιστο 0.70 leaderboard score.
