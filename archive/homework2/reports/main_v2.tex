...
Το final score στο Kaggle public leaderboard ήταν \textbf{0.70 macro-F1}, με leaderboard name \textbf{AlexTuring - sdi2200284}. Στο τοπικό μου validation το καλύτερο ensemble έφτασε \textbf{0.7136 macro-F1}. Η διαφορά τοπικού-Kaggle είναι λογική γιατί το test split του Kaggle δεν είναι ακριβώς η ίδια κατανομή με το δικό μου val.
...
Στο public leaderboard εμφανίζομαι ως \textbf{AlexTuring - sdi2200284}. Το public αποτέλεσμα που κρατάω ως τελικό για την αναφορά είναι \textbf{0.70 macro-F1}. Το private leaderboard μπορεί φυσικά να αλλάξει λίγο την εικόνα μετά το τέλος του διαγωνισμού, επειδή αξιολογείται σε διαφορετικό split.
...
Με 3 seeds (42, 0, 1) και alpha-weighted average των probabilities (alpha = $[0.9, 0.0, 0.1]$ optimized στο val), το score έγινε \textbf{0.7057}. Αυτό υπεβλήθη στο Kaggle: \textbf{0.70 public}.
...
Δεν την έβαλα ως final submission. Έμεινα στο 0.70 από το πιο safe Phase 12 alpha (που είχε balanced predictions).
...
Η εργασία αυτή έβαλε σε δοκιμή το πόσο μπορούν off-the-shelf pretrained Transformer μοντέλα να λύσουν ένα πραγματικό, μεσαίας δυσκολίας NLP task. Από 0.6017 (HW1 baseline) σε 0.70 public Kaggle και 0.7136 local val. Κερδισμένα +0.11 macro-F1.
